...
import app.family.BuildConfig;
import app.family.Detail;
import app.family.F;
import app.family.Gallery;
import app.family.Global;
import app.family.Board;
import app.family.Memory;
import app.family.R;
import app.family.U;
import app.family.visit.ReferencesMedia;
import static app.family.Global.gc;

public class Immagine extends Detail {
	...
	public void impagina() {
		...
		if( Global.settings.expert ) metti(getString(R.string.file), "File");    // 'Angelina Guadagnoli.jpg' visibile solo agli esperti
		...
		ReferencesMedia riferiMedia = new ReferencesMedia(gc, m, false);
	...
	void immaginona(Media m, int posizione) {
		vistaMedia = LayoutInflater.from(this).inflate(R.layout.image_image, box, false);
		...
		vistaMedia.setOnClickListener(vista -> {
			...
			if( tipoFile == 0 ) {    // Il file è da trovare
				...
				Intent intento = new Intent( Immagine.this, Board.class );
	...
	@Override
	public void elimina() {
		U.aggiornaDate( Gallery.eliminaMedia( m, null ) );
	}
}