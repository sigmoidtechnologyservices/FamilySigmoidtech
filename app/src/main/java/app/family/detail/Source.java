...
public class Source extends Detail {

	org.folg.gedcom.model.Source f;

	@Override
	public void impagina() {
		setTitle( R.string.source );
		f = (org.folg.gedcom.model.Source) casta( org.folg.gedcom.model.Source.class );
		mettiBava( "SOUR", f.getId() );
		ListQuotesSource citazioni = new ListQuotesSource( gc, f.getId() );
		...
		if( f.getRepositoryRef() != null ) {
			View vistaRef = LayoutInflater.from(this).inflate( R.layout.piece_citation_source, box, false );
			...
			vistaRef.setOnClickListener( new View.OnClickListener() {
				public void onClick( View v ) {
					...
					startActivity( new Intent( Source.this, Archive_Ref.class ) );
				}
			} );
	...
	@Override
	public void elimina() {
		U.aggiornaDate( Library.eliminaFonte(f) );
	}
}
