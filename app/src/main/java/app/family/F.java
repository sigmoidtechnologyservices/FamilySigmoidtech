...
public class F {
		...
		protected Bitmap doInBackground(String... parametri) {
			...
			try {
				...
				if( lista.isEmpty() ) { // Pagina web trovata ma senza immagini
					...
					return generaIcona( vistaImmagine, R.layout.media_world, url.getProtocol() );	// ritorna una bitmap
				}
				...
				(dialog, id) -> {
					...
					if( intento.getComponent().getPackageName().equals("app.familygem") ) {
						...
						if( codice==4173 || codice==2173 ) { // Media semplice
							...
							med = Gallery.nuovoMedia( contenitore );
							Memory.setPrimo( med );
						}
						med.setFile( "" );
	...
	static void concludiProponiRitaglio( Context contesto, Fragment frammento ) {
		if( frammento instanceof Gallery)
			((Gallery)frammento).ricrea();
		else if( contesto instanceof Detail)
			((Detail)contesto).refresh();
		else if( contesto instanceof Individual) {
