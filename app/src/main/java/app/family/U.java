...
import app.family.constants.Format;
...
public class U {
	...
	static void gedcomSicuro(Gedcom gc) {
		if( gc == null )
			Global.gc = Trees.leggiJson(Global.settings.openTree);
	}
	...
	public static boolean areMarried(org.folg.gedcom.model.Family family) {
		if( family != null ) {
	...
	public static boolean areDivorced(org.folg.gedcom.model.Family family) {
		if( family != null ) {
			for( EventFact eventFact : family.getEventsFacts() ) {
	...
	public static String nuovoId(Gedcom gc, Class classe) {
		...
		if( classe == Note.class ) {
			...
			for( Media m : gc.getMedia() )
				calcolaMax(m);
		} else if( classe == org.folg.gedcom.model.Source.class ) {
			pre = "S";
			for( org.folg.gedcom.model.Source f : gc.getSources() )
				calcolaMax(f);
		} else if( classe == Person.class ) {
			pre = "I";
			for( Person p : gc.getPeople() )
				calcolaMax(p);
		} else if( classe == org.folg.gedcom.model.Family.class ) {
			pre = "F";
	...
	public static void metti(LinearLayout scatola, String tit, String testo) {
		View vistaPezzo = LayoutInflater.from(scatola.getContext()).inflate(R.layout.piece_made, scatola, false);
	...
	static String details(Person person, TextView detailsView) {
		...
		String places = Registry.twoPlaces(person);
	...
	public static View mettiIndividuo( LinearLayout scatola, Person persona, String ruolo ) {
		View vistaIndi = LayoutInflater.from(scatola.getContext()).inflate(R.layout.piece_individual, scatola, false);
	...
	static void mettiNota(final LinearLayout scatola, final Note nota, boolean dettagli) {
		...
		View vistaNota = LayoutInflater.from(contesto).inflate(R.layout.known_piece, scatola, false);
		...
		if( dettagli ) {
			...
			if( contesto instanceof Individual) { // Fragment individuoEventi
			...
			vistaNota.setOnClickListener(v -> {
				if( nota.getId() != null )
					Memory.setPrimo(nota);
				else
					Memory.aggiungi(nota);
	...
	public static Object[] eliminaNota(Note note, View view) {
		...
		if( note.getId() != null ) { // OBJECT note
			...
			References_Note eliminatoreNote = new References_Note(Global.gc, note.getId(), true);
	...
	public static void mettiMedia(LinearLayout scatola, Object contenitore, boolean dettagli) {
		RecyclerView griglia = new AdapterGalleryMedia.RiciclaVista(scatola.getContext(), dettagli);
		...
		AdapterGalleryMedia adattatore = new AdapterGalleryMedia(listaMedia, dettagli);
	...
	public static void citaFonti(LinearLayout layout, Object container) {
		if( Global.settings.expert ) {
			...
			for( final SourceCitation citaz : listaCitaFonti ) {
				View vistaCita = LayoutInflater.from(layout.getContext()).inflate(R.layout.piece_citation_source, layout, false);
				layout.addView(vistaCita);
				if( citaz.getSource(Global.gc) != null )    // source CITATION
					((TextView)vistaCita.findViewById(R.id.fonte_testo)).setText(Library.titoloFonte(citaz.getSource(Global.gc)));
				else // source NOTE, oppure Citazione di source che è stata eliminata
				...
				if( layout.getContext() instanceof Individual) { // Fragment individuoEventi
				...
				vistaCita.setOnClickListener(v -> {
					Intent intento = new Intent(layout.getContext(), QuoteSource.class);
	...
	// Inserisce nella scatola il richiamo ad una source, con dettagli o essenziale
	public static void mettiFonte(final LinearLayout scatola, final org.folg.gedcom.model.Source source, boolean dettagli ) {
		View vistaFonte = LayoutInflater.from(scatola.getContext()).inflate( R.layout.piece_source, scatola, false );
		...
		if( dettagli ) {
			...
			mettiNote( scatolaAltro, source, false );
			mettiMedia( scatolaAltro, source, false );
			...
			txt = Library.titoloFonte(source);
		}
		vistaTesto.setText( txt );
		vistaFonte.setOnClickListener( v -> {
			Memory.setPrimo(source);
			scatola.getContext().startActivity( new Intent( scatola.getContext(), Source.class) );
		});
	}

	// La view ritornata è usata da Condivisione
	public static View linkaPersona(LinearLayout scatola, Person p, int scheda) {
		View vistaPersona = LayoutInflater.from(scatola.getContext()).inflate(R.layout.piece_individual_small, scatola, false);
		...
		vistaPersona.setOnClickListener(v -> {
			Memory.setPrimo(p);
			Intent intento = new Intent(scatola.getContext(), Individual.class);
	...
	static String testoFamiglia(Context contesto, Gedcom gc, org.folg.gedcom.model.Family fam, boolean unaLinea ) {
		String testo = "";
	...
	// Usato da dispensa
	static void linkaFamiglia(LinearLayout scatola, org.folg.gedcom.model.Family fam ) {
		View vistaFamiglia = LayoutInflater.from(scatola.getContext()).inflate( R.layout.piece_family_small, scatola, false );
	...
	static void linkaMedia( LinearLayout scatola, Media media ) {
		View vistaMedia = LayoutInflater.from(scatola.getContext()).inflate( R.layout.average_piece, scatola, false );
		scatola.addView( vistaMedia );
		AdapterGalleryMedia.arredaMedia( media, vistaMedia.findViewById(R.id.media_testo), vistaMedia.findViewById(R.id.media_num) );
	...
	static void linkAutore( LinearLayout scatola, Submitter autor ) {
		...
		View vista = LayoutInflater.from(contesto).inflate( R.layout.known_piece, scatola, false);
		...
		vista.setOnClickListener( v -> {
			...
			contesto.startActivity( new Intent( contesto, Author.class ) );
		});
	}
	...
	static void mettiQualsiasi(LinearLayout scatola, Object record) {
		...
		else if( record instanceof org.folg.gedcom.model.Source)
			mettiFonte(scatola, (org.folg.gedcom.model.Source)record, false);
		else if( record instanceof org.folg.gedcom.model.Family)
			linkaFamiglia(scatola, (org.folg.gedcom.model.Family)record);
		else if( record instanceof Repository )
			Archive_Ref.mettiArchivio(scatola, (Repository)record);
	...
	public static View cambiamenti(final LinearLayout layout, final Change change) {
		...
		if( change != null && Global.settings.expert ) {
			changeView = LayoutInflater.from(layout.getContext()).inflate(R.layout.piece_date_changes, layout, false);
			...
			if( change.getDateTime() != null ) {
				...
				if( change.getDateTime().getValue() != null )
					txt = new Dater(change.getDateTime().getValue()).writeDateLong();
			...
			changeView.setOnClickListener(v -> {
				...
				layout.getContext().startActivity(new Intent(layout.getContext(), Changes.class));
			});
		}
		return changeView;
	}
	...
	static boolean ciSonoIndividuiCollegabili(Person person) {
		...
		if( total > 0 && (Global.settings.expert // gli esperti possono sempre
			...
			return true;
		int kin = Registry.countRelatives(person);
		return total > kin + 1;
	}
		...
		if( testa[0] == null || testa[0].getSubmitterRef() == null ) {
			new AlertDialog.Builder( contesto ).setMessage( R.string.make_main_submitter )
					.setPositiveButton( android.R.string.yes, (dialog, id) -> {
						if( testa[0] == null ) {
							testa[0] = TreeNew.creaTestata( Global.settings.openTree +".json" );
	...
	static String[] elencoFamiglie(List<org.folg.gedcom.model.Family> listaFamiglie) {
	...
	public static void qualiGenitoriMostrare( Context contesto, Person perno, int cosaAprire ) {
		if( perno == null )
			...
			List<org.folg.gedcom.model.Family> famiglie = perno.getParentFamilies(Global.gc);
	...
	private static void concludiSceltaGenitori( Context contesto, Person perno, int cosaAprire, int qualeFamiglia ) {
		...
		if( cosaAprire < 2 ) { // Mostra il diagramma
			...
			org.folg.gedcom.model.Family family = perno.getParentFamilies(Global.gc).get(qualeFamiglia);
			if( contesto instanceof Family) { // Passando di Famiglia in Famiglia non accumula attività nello stack
				Memory.replacePrimo(family);
				((Activity)contesto).recreate();
			} else {
				Memory.setPrimo(family);
				contesto.startActivity(new Intent(contesto, Family.class));
			}
		}
	}

	// Per un perno che ha molteplici matrimoni chiede quale mostrare
	public static void qualiConiugiMostrare(Context contesto, Person perno, org.folg.gedcom.model.Family family) {
		if( perno.getSpouseFamilies(Global.gc).size() > 1 && family == null ) {
			new AlertDialog.Builder( contesto ).setTitle( R.string.which_family )
					.setItems( elencoFamiglie(perno.getSpouseFamilies(Global.gc)), (dialog, quale) -> {
						concludiSceltaConiugi( contesto, perno, null, quale );
					}).show();
		} else {
			concludiSceltaConiugi( contesto, perno, family, 0 );
		}
	}
	private static void concludiSceltaConiugi(Context contesto, Person perno, org.folg.gedcom.model.Family family, int quale) {
		Global.indi = perno.getId();
		family = family == null ? perno.getSpouseFamilies(Global.gc).get(quale) : family;
		if( contesto instanceof Family) {
			Memory.replacePrimo(family);
	...
	static boolean controllaMultiMatrimoni( Intent intento, Context contesto, Fragment frammento ) {
		...
		List<org.folg.gedcom.model.Family> famGenitori = perno.getParentFamilies(Global.gc);
		List<org.folg.gedcom.model.Family> famSposi = perno.getSpouseFamilies(Global.gc);
		int relazione = intento.getIntExtra( "relazione", 0 );
		ArrayAdapter<New_Relative.VoceFamiglia> adapter = new ArrayAdapter<>(contesto, android.R.layout.simple_list_item_1);
		...
		if( relazione == 1 && famGenitori.size() > 1 ) {
			for( org.folg.gedcom.model.Family fam : famGenitori )
				if( fam.getHusbandRefs().isEmpty() || fam.getWifeRefs().isEmpty() )
					adapter.add( new New_Relative.VoceFamiglia(contesto,fam) );
		...
		// Coniuge
			...
			for( org.folg.gedcom.model.Family fam : famSposi ) {
				if( fam.getHusbandRefs().isEmpty() || fam.getWifeRefs().isEmpty() )
					adapter.add( new New_Relative.VoceFamiglia(contesto,fam) );
			}
	...
	static boolean controllaFamiglieVuote(Context contesto, Runnable cheFare, boolean ancheKo, org.folg.gedcom.model.Family... famiglie) {
		List<org.folg.gedcom.model.Family> vuote = new ArrayList<>();
		for( org.folg.gedcom.model.Family fam : famiglie ) {
		...
		if( vuote.size() > 0 ) {
			new AlertDialog.Builder(contesto).setMessage(R.string.empty_family_delete)
					.setPositiveButton(android.R.string.yes, (dialog, i) -> {
						for( org.folg.gedcom.model.Family fam : vuote )
							church.deleteFamily(fam); // Così capita di salvare più volte insieme... ma vabè
