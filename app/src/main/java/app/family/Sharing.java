...
public class Sharing extends AppCompatActivity {
		...
		if( gc != null ) {
			...
			findViewById( R.id.bottone_condividi ).setOnClickListener( v -> {
				if( uploadSuccesso )
					...
					if( header == null ) {
						header = TreeNew.creaTestata(tree.id + ".json");
						gc.setHeader(header);
					} else
						header.setDateTime(U.dataTempoAdesso());
					if( autore[0] == null ) {
						autore[0] = Podium.nuovoAutore(null);
					}
	...
	static class PostaDatiCondivisione extends AsyncTask<Sharing,Void, Sharing> {
		@Override
		protected Sharing doInBackground(Sharing... contesti) {
			Sharing questo = contesti[0];
			try {
		...
		@Override
		protected void onPostExecute(Sharing questo) {
	...
	static class InvioFTP extends AsyncTask<Sharing, Void, Sharing> {
		protected Sharing doInBackground(Sharing... contesti) {
			Sharing questo = contesti[0];
			try {
