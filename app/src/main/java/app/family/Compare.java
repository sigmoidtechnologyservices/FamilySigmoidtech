// Introduction to importing news in an existing tree
...
public class Compare extends AppCompatActivity {
	...
	protected void onCreate( Bundle bandolo ) {
		...
		Global.gc = Trees.apriGedcomTemporaneo( idAlbero, true );
		Global.gc2 = Trees.apriGedcomTemporaneo( idAlbero2, false );
		...
		if( Comparison.getLista().size() > 0 ) {
			...
			botton1.setOnClickListener(v -> {
				startActivity(new Intent(Compare.this, Comparator.class).putExtra("posizione", 1));
			});
			// Accetta tutto
			botton2.setOnClickListener(v -> {
				...
				Intent intent = new Intent(Compare.this, Comparator.class);
			...
			botton1.setOnClickListener(v -> {
				Trees.deleteTree(Compare.this, idAlbero2);
				onBackPressed();
			});
	...
	void arredaScheda( Gedcom gc, int idAlbero, int idScheda ) {
		...
		((TextView)carta.findViewById(R.id.confronto_testo)).setText(Trees.scriviDati(this, tree));
