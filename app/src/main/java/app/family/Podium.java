...
public class Podium extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle stato) {
		...
		for( final Submitter autor : listAutori ) {
			...
			((TextView)vistaPezzo.findViewById(R.id.magazzino_nome)).setText(InfoTree.nomeAutore(autor));
			...
			vistaPezzo.setOnClickListener(v -> {
				...
				startActivity(new Intent(getContext(), Author.class));
			});
	...
	static void autorePrincipale(Submitter subm) {
		Header testa = gc.getHeader();
		if( testa == null ) {
			testa = TreeNew.creaTestata(Global.settings.openTree + ".json");
			gc.setHeader(testa);
		}
