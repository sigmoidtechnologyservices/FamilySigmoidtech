...
import app.family.constants.Kind;

public class PublisherDate extends LinearLayout {
	...
	public PublisherDate(Context contesto, AttributeSet as ) {
		super( contesto, as );
	}
	...
	void inizia( final EditText editaTesto ) {
		...
		dater = new Dater( editaTesto.getText().toString() );
		data1 = dater.data1;
		data2 = dater.data2;
		...
		if( Global.settings.expert ) {
			...
			elencoTipi.setOnClickListener( vista -> {
				...
				popup.setOnMenuItemClickListener(item -> {
					dater.kind = Kind.values()[item.getItemId()];
					...
					if( dater.kind == Kind.BETWEEN_AND || dater.kind == Kind.FROM_TO ) {
			...
			findViewById(R.id.editadata_circa).setOnClickListener(vista -> {
				...
				dater.kind = ((CompoundButton)vista).isChecked() ? Kind.APPROXIMATE : Kind.EXACT;
				veroImputTesto = false;
				genera();
			});
		...
		editaTesto.setOnFocusChangeListener((v, ciapaFocus) -> {
			if( ciapaFocus ) {
				if( dater.kind == Kind.PHRASE ) {
					//genera(); // Toglie le parentesi alla frase
					editaTesto.setText(dater.frase);
				} else {
				...
				dater.data1.date = null; // un resettino
				impostaTutto();
	...
	void impostaTutto() {
		dater.analizza( editaTesto.getText().toString() );
		((CheckBox)findViewById( R.id.editadata_circa )).setChecked( dater.kind == Kind.APPROXIMATE );
		((TextView)findViewById( R.id.editadata_tipi )).setText( dateKinds[dater.kind.ordinal()] );
	...
	void impostaCarro(Dater.Data data, NumberPicker ruotaGiorno, NumberPicker ruotaMese, NumberPicker ruotaSecolo, NumberPicker ruotaAnno) {
		calenda.clear();
		if( data.date != null )
	...
	void impostaCecchi(Dater.Data data) {
	...
	void aggiorna(Dater.Data data, NumberPicker ruotaGiorno, NumberPicker ruotaMese, NumberPicker ruotaSecolo, NumberPicker ruotaAnno ) {
