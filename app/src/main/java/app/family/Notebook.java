...
public class Notebook extends Fragment {
	...
	public View onCreateView( LayoutInflater inflater, ViewGroup container, Bundle bandolo ) {
		...
		if( gc != null ) {
			...
			if( !listaNoteCondivise.isEmpty() ) {
				if( !scegliNota ) {
					View tit = inflater.inflate( R.layout.title, scatola, true );
			...
			// Note inlinea
			ListNote visitaNote = new ListNote();
			if( !scegliNota ) {
				gc.accept( visitaNote );
				if( !visitaNote.listaNote.isEmpty() ) {
					View tit = inflater.inflate( R.layout.title, scatola, false );
	...
	View mettiNota( final LinearLayout scatola, final Note nota ) {
		View vistaNota = LayoutInflater.from(scatola.getContext()).inflate( R.layout.notebook_piece, scatola, false );
		...
		if( nota.getId() == null )
			...
			References_Note contaUso = new References_Note( gc, nota.getId(), false );
