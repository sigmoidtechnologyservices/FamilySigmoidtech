...
/*
"degree":
0	tree created from scratch
it remains 0 even adding the main submitter, sharing it and receiving news
9 tree sent for sharing pending marking all submitters 'past'
10 tree received via sharing in Australia
it can never return 0 again
20 tree returned to Italy proved to be a derivative of a zero (or a 10).
only if it is 10 it can become 20. If by chance it loses the status of derivative it returns 10 (never 0)
30 derived tree from which all novelties have been extracted OR without novelties already upon arrival (gray). Disposable
*/
