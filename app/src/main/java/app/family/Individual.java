...
public class Individual extends AppCompatActivity {
	...
	protected void onCreate(Bundle bundle) {
		...
		if( uno == null && bundle != null ) {
			...
			Memory.setPrimo(uno); // Altrimenti la memoria è senza una pila
		}
	...
	class ImpaginatoreSezioni extends FragmentPagerAdapter {
		...
		public Fragment getItem( int position ) {
			...
			if( position == 0 )
				...
				scheda = new IndividualEvents();
			else if( position == 2 )
				scheda = new IndividualFamily();
			return scheda;
		}
	...
	protected void onStart() {
		...
		findViewById(R.id.fab).setOnClickListener(vista -> {
			PopupMenu popup = new PopupMenu(Individual.this, vista);
			...
			popup.setOnMenuItemClickListener(item -> {
				...
				switch( item.getItemId() ) {
					...
					case 10: // Cerca media locale
						F.appAcquisizioneImmagine(Individual.this, null, 2173, uno);
						break;
					case 11: // Cerca oggetto media
						F.appAcquisizioneImmagine(Individual.this, null, 2174, uno);
						break;
					case 12: // Collega media in Galleria
						Intent inten = new Intent(Individual.this, Principal.class);
					...
					case 20: // Crea nome
						...
						nome.setValue("//");
						uno.addName(nome);
						Memory.aggiungi(nome);
						startActivity(new Intent(Individual.this, Nome.class));
						break;
					case 21: // Crea sesso
						...
						new AlertDialog.Builder( tabLayout.getContext() )
								.setSingleChoiceItems( sessoNomi, -1, ( dialo, i ) -> {
									...
									dialo.dismiss();
									IndividualEvents.aggiornaRuoliConiugali(uno);
									IndividualEvents tabEventi = (IndividualEvents) getSupportFragmentManager().findFragmentByTag( "android:switcher:" + R.id.schede_persona + ":1" );
									tabEventi.refresh(1);
					...
					case 22: { // Crea nota
						...
						startActivity( new Intent( Individual.this, Nota.class ) );
						// todo? Dettaglio.edita( View vistaValore );
						break;
					}
					case 23: // Crea nota condivisa
						Notebook.nuovaNota( Individual.this, uno );
						break;
					case 24: // Collega nota condivisa
						Intent inte = new Intent( Individual.this, Principal.class );
						inte.putExtra( "quadernoScegliNota", true );
						startActivityForResult( inte,4074 );
						break;
					case 25: // Nuova source-nota
						SourceCitation citaz = new SourceCitation();
						citaz.setValue("");
						uno.addSourceCitation(citaz);
						Memory.aggiungi(citaz);
						startActivity(new Intent(Individual.this, QuoteSource.class));
						break;
					case 26: // Nuova source
						Library.nuovaFonte(Individual.this, uno);
						break;
					case 27: // Collega source
						Intent intento = new Intent(Individual.this, Principal.class);
					...
					case 30:// Collega persona nuova
						if( Global.settings.expert ) {
							DialogFragment dialog = new New_Relative(uno, null, null, true, null);
							...
							builder.setItems(familiari, (dialog, quale) -> {
								...
								if( U.controllaMultiMatrimoni(intento1, Individual.this, null) )
									return;
					...
					case 31: // Collega persona esistente
						if( Global.settings.expert ) {
							DialogFragment dialog = new New_Relative(uno, null, null, false, null);
							dialog.show(getSupportFragmentManager(), "scegli");
						} else {
							builder.setItems(familiari, (dialog, quale) -> {
								...
								if( U.controllaMultiMatrimoni(intento2, Individual.this, null) )
									return;
					...
					default:
						...
						if( keyTag != null ) {
							...
							startActivity(new Intent(Individual.this, Event.class));
							U.salvaJson(true, uno);
							return true;
						}
						return false;
	...
	public void onActivityResult( int requestCode, int resultCode, Intent data ) {
		...
		if( resultCode == RESULT_OK ) {
			if( requestCode == 2173 ) { // File fornito da un'app diventa media locale eventualmente ritagliato con Android Image Cropper
				...
				Media media = Gallery.nuovoMedia( uno );
			...
			case 5:	// Elimina
				new AlertDialog.Builder(this).setMessage(R.string.really_delete_person)
						.setPositiveButton(R.string.delete, (dialog, i) -> {
							Family[] famiglie = Registry.eliminaPersona(this, uno.getId());
