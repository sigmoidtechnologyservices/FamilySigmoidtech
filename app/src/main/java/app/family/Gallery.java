...
public class Gallery extends Fragment {
	...
	AdapterGalleryMedia adattatore;
	...
	public View onCreateView( LayoutInflater inflater, ViewGroup container, Bundle bandolo ) {
		...
		if( gc != null ) {
			...
			adattatore = new AdapterGalleryMedia( visitaMedia.listaMedia, true );
			griglia.setAdapter( adattatore );
			vista.findViewById( R.id.fab ).setOnClickListener( v ->
					F.appAcquisizioneImmagine( getContext(), Gallery.this, 4546, null )
			);
		}
		return vista;
	}
	...
	static int popolarita( Media med ) {
		ReferencesMedia riferiMedia = new ReferencesMedia( gc, med, false );
	...
	public static Object[] eliminaMedia(Media media, View vista) {
		...
		if( media.getId() != null ) { // media OBJECT
			...
			ReferencesMedia eliminaMedia = new ReferencesMedia(gc, media, true);
			capi = eliminaMedia.capostipiti;
		} else { // media LOCALE
			new FindPila(gc, media); // trova temporaneamente la pila del media per individuare il container
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		if( item.getItemId() == 0 ) {
			startActivity(new Intent(getContext(), FoldersMedia.class)
