...
public class IndividuoMedia extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		View vistaMedia = inflater.inflate(R.layout.individual_card, container, false);
		if( gc != null ) {
			...
			if( uno != null ) {
				...
				AdapterGalleryMedia adattatore = new AdapterGalleryMedia(visitaMedia.listaMedia, true);
	...
	public boolean onContextItemSelected( MenuItem item ) {
		...
		if( id == 0 ) { // Principale
			...
			Gallery.scollegaMedia(media.getId(), (MediaContainer)container);
			U.salvaJson(true, uno);
			refresh();
			return true;
		} else if( id == 2 ) { // Elimina
			Object[] capi = Gallery.eliminaMedia(media, null);
	...
	void refresh() {
		...
		// Scheda eventi
		IndividualEvents tabEventi = (IndividualEvents)requireActivity().getSupportFragmentManager().findFragmentByTag("android:switcher:" + R.id.schede_persona + ":1");
		tabEventi.refresh(1);
	}
}
