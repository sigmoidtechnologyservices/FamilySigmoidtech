...
public class IndividualEvents extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		View vistaEventi = inflater.inflate(R.layout.individual_card, container, false);
		if( gc != null ) {
			...
			if( uno != null ) {
				...
				for (EventFact fatto : uno.getEventsFacts() ) {
					...
					if( fatto.getDate() != null ) txt += new Dater(fatto.getDate()).writeDateLong() + "\n";
					if( fatto.getPlace() != null ) txt += fatto.getPlace() + "\n";
					Address indirizzo = fatto.getAddress();
					if( indirizzo != null ) txt += Detail.writeAddress(indirizzo, true) + "\n";
				...
				for( Extension est : U.trovaEstensioni( uno ) ) {
	...
	private void piazzaEvento(LinearLayout scatola, String titolo, String testo, Object oggetto) {
		View vistaFatto = LayoutInflater.from(scatola.getContext()).inflate( R.layout.individual_events_piece, scatola, false);
		...
		if( oggetto instanceof Name ) {
			...
			vistaFatto.setOnClickListener( v -> {
				Memory.aggiungi( oggetto );
				startActivity( new Intent( getContext(), app.family.detail.Extension.class ) );
			});
		}
	}
	...
	public boolean onContextItemSelected( MenuItem item ) {
		...
		switch( item.getItemId() ) {
			...
			case 230: // Copia
				...
				return true;
			case 231: // Elimina
				// todo conferma : Vuoi eliminare questa citazione della source? La source continuerà ad esistere.
